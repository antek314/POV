...
//#define NP32_WS2812_ZERO_PERIODS 50

#define NP32_PWM_HIGH 60
#define NP32_PWM_LOW  30

//#define NP32_RESOLVE_BIT_TIME(color_byte, bit) (((color_byte >> bit) & 0x01) ? NP32_PWM_HIGH : NP32_PWM_LOW)
...
/*void init_neopixel(void)
{
    ...
    np_handle.LED_Count = 8; // ilość LED
    //instance.StartDMA_Call = StartDMA;

    np_handle.StartDMA_Call = Start_DMA;

    if (NP32_Init(&np_handle) != 0)
    {
        Error_Handler();
    }

    NP32_RGB_t kolor = { .R = 0, .G = 0, .B = 255 };
    NP32_SetAllLEDs_RGB(&np_handle, kolor);
    NP32_Update(&np_handle);


    NP32_RGB_t blue = { .R = 0, .G = 0, .B = 255 };

    NP32_SetLED_RGB(&np_handle, 0, blue);
    NP32_SetLED_RGB(&np_handle, 1, blue);
    NP32_SetLED_RGB(&np_handle, 2, blue);
    NP32_SetLED_RGB(&np_handle, 3, blue);
