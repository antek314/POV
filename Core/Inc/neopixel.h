...
struct __NP32_Instance
{
    ...
    void (*StartDMA_Call)(uint16_t *dma_buf, uint16_t length); /**< Function delegate for starting the DMA stream from the
...
//int8_t Start_DMA(uint16_t *data, uint32_t size);

/*typedef struct {
    // …
    int8_t (*StartDMA_Call)(uint16_t *dma_buffer, uint32_t length);
    // …
} NP32_Instance_t;*/
